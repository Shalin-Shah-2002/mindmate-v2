...
class ProfileView extends StatelessWidget {
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: SingleChildScrollView(
          ...
          child: Obx(() {
            // Try to find or initialize the auth controller
            if (authController.userModel != null) {
              ...
              // We have Firebase user but no UserModel - trigger loading
              WidgetsBinding.instance.addPostFrameCallback((_) async {
                if (authController.userModel == null && authController.user != null) {
                  try {
                    final authService = AuthService();
                    final userProfile = await authService.getUserProfile(authController.user!.uid);
                    if (userProfile != null) {
                      // Manually update the user model by calling a method
                      authController.updateUserModel(userProfile);
                    }
                  } catch (e) {
  ...
  Widget _buildQuickStatsRow() {
    return Row(
      children: [
        Expanded(
          child: _buildQuickStatCard('Posts', '3', Icons.article, const Color(0xFFEC4899)),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _buildQuickStatCard('Likes', '24', Icons.favorite, const Color(0xFFEF4444)),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _buildQuickStatCard('Days', '15', Icons.calendar_today, const Color(0xFF10B981)),
        ),
        const SizedBox(width: 12),
        Expanded(
          child: _buildQuickStatCard('Level', '2', Icons.star, const Color(0xFFF59E0B)),
        ),
      ],
    );
  }
  ...
  Widget _buildEnhancedSectionCard(String title, IconData icon, Color color, Widget content) {
    return Container(
      width: double.infinity,
      ...
      Obx(() {
        ...
        WidgetsBinding.instance.addPostFrameCallback((_) {
          if (communityController.userPosts.isEmpty &&
            ...
            if (authController.userModel != null) {
              communityController.loadUserPosts(authController.userModel!.id);
            }
          }
        });
  ...
  Widget _buildEmptyPostsState(CommunityViewModel communityController) {
    return Column(
      children: [
        ...
        ElevatedButton(
          onPressed: () {
            Get.find<NavigationViewModel>().changeTab(1); // Community tab index
          },
          style: ElevatedButton.styleFrom(
        ...
        children: [
          ...
          Row(
            children: [
              ...
              const SizedBox(width: 16),
              _buildPostStat(Icons.chat_bubble, post.commentsCount, Colors.blue),
              const Spacer(),
              Container(
